...
from transformers import AutoModelForCausalLM, AutoTokenizer, AutoModelForSequenceClassification, RobertaConfig, RobertaModel
...
class PromptCS(nn.Module):
    def __init__(self, args, device, template):
        ...
        self.model = LLM.create_model(self.args, self.use_lm_finetune)
    ...
    def embed_input(self, queries):
        if self.mode == 'PromptCS':
            return self.PromptCS_embed_input(queries)
        else:
    ...
    def PromptCS_embed_input(self, queries):
        bz = queries.shape[0]
        pure_queries = queries.clone()
        pure_queries[(queries == self.pseudo_token_id)] = self.unk_token_id
        raw_embeds = self.embeddings(pure_queries)
...
class PromptAgent(torch.nn.Module):
    def __init__(self, template, hidden_size, tokenizer, device, args):
        ...
        self.cloze_mask = [
            [1] * self.cloze_length[0]  # front end
            + [1] * self.cloze_length[1]  # back end
        ]
        self.prompt_length = template[0] + template[1]
        self.cloze_mask = torch.LongTensor(self.cloze_mask).bool().to(self.device)
        self.seq_indices = torch.LongTensor(list(range(self.prompt_length))).to(self.device)
        ...
        if args.prompt_encoder_type == "lstm":
            ...
            self.prompt_encoder = Encoder_Transformer(d_model=self.hidden_size,
                                                  nhead=8,
                                                  num_layers=6,
                                                  max_len=self.prompt_length)

        self.embedding = torch.nn.Embedding(self.prompt_length, self.embed_size).to(self.device)
...
class LLM(nn.Module):
    def create_model(args, use_lm_finetune):
        ...
        for param in model.parameters():
            param.requires_grad = use_lm_finetune
